...
function actionColumn({ csrf, scene, patch, print, clear, css: { body = () => {}, head = () => {} } = {} } = {}) {
    const props = `${csrf ? `csrf="${csrf}"` : ""}${scene ? `scene="${scene}"` : ""}${patch ? `patch="${patch}"` : ""}${print ? `print="${print}"` : ""}${clear ? `clear="${clear}"` : ""}`;
    return {
        name: "action",
        text: $trans("Actions"),
...
function searchable (field, route, colMapper, opts) {
    ...
    if(opts) {
        field.empty = function () {
            return `<div part="add-new"><span part="empty-data">${$trans("No data found")}</span><a part="add-new-link" href="${opts.link}">${opts.text ? opts.text : $trans("Add new")}</a></div>`;
        }
    }
}

function TableVisualizer ({
    dataVisualizer, colMapper, routes, rowClick, colClick
}) {
    var timer;
    const _routes = {
        ...$routes,
    ...
    Links.innerHTML = `<neo-tooltip id="prev" slot="end"><a class="flex w-8 h-8 items-center justify-center text-x-black outline-none rounded-x-thin !bg-opacity-5 hover:bg-x-black focus:bg-x-black focus-within:bg-x-black"><svg class="block w-6 h-6 pointer-events-none" fill="currentcolor" viewBox="0 -960 960 960"><path d="M452-219 190-481l262-262 64 64-199 198 199 197-64 65Zm257 0L447-481l262-262 63 64-198 198 198 197-63 65Z" /></svg></a></neo-tooltip><neo-tooltip id="next" slot="end"><a class="flex w-8 h-8 items-center justify-center text-x-black outline-none rounded-x-thin !bg-opacity-5 hover:bg-x-black focus:bg-x-black focus-within:bg-x-black"><svg class="block w-6 h-6 pointer-events-none" fill="currentcolor" viewBox="0 -960 960 960"><path d="M388-481 190-679l64-64 262 262-262 262-64-65 198-197Zm257 0L447-679l63-64 262 262-262 262-63-65 198-197Z" /></svg></a></neo-tooltip>`;
    ...
    function createLinks (prev, next, str) {
        ...
        if(prev) {
            ...
            if(preva) preva.children[0].href = href
                ...
                _preva.children[0].addEventListener("click", event);
            ...
            if(preva) {
                preva.children[0].removeEventListener("click", event);
                preva.remove();
            }
        }
        if(next) {
            ...
            if(nexta) nexta.children[0].href = href
                ...
                _nexta.children[0].addEventListener("click", event);
            ...
            if(nexta) {
                nexta.children[0].removeEventListener("click", event);
                nexta.remove();
            }
        }
    }
...
const
    ...
    $flip = (array) => document.documentElement.dir === "rtl" ? array.reverse() : array,
...
Neo.load(function () {
    $queryAll("neo-stepper").forEach(stepper => {
        const tabPrev = $query("#prev", stepper),
            tabSave = $query("#save", stepper),
            tabNext = $query("#next", stepper);

        stepper.addEventListener("next", (e) => {
            e.preventDefault();
            Neo.Validator.validate(e.detail.step, {
                ...($queryAll("[rules]", e.detail.step).reduce((carry, item) => {
                ...
                execute () {
                    stepper.step = stepper.steps[stepper.current + 1].step;
                }
            });
        });

        stepper.addEventListener("change:step", e => {
            tabPrev.style.display = stepper.current <= 0 ? "none" : "";
            tabNext.style.display = stepper.current < stepper.steps.length - 1 ? "" : "none";
            tabSave.style.display = stepper.current === stepper.steps.length - 1 ? "" : "none";
        });
