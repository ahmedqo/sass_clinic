...
@section('content')
    <form validate action="{{ route('actions.profile.patch') }}" method="POST" class="w-full">
        @csrf
        @method('patch')
            ...
            <neo-stepper-item step="step-1" class="grid grid-cols-1 grid-rows-1 gap-6">
                <div class="flex flex-col gap-1">
                    ...
                    <neo-textbox rules="required" errors='{"required": "{{ __('The first name field is required') }}"}'
                        ...
                        value="{{ old('first_name', $data->first_name) }}"></neo-textbox>
                </div>
                <div class="flex flex-col gap-1">
                    <label class="text-x-black font-x-thin text-base">
                        {{ __('Last name') }} (*)
                    ...
                    <neo-textbox rules="required" errors='{"required": "{{ __('The last name field is required') }}"}'
                        ...
                        value="{{ old('last_name', $data->last_name) }}"></neo-textbox>
                </div>
                <div class="flex flex-col gap-1">
                    <label class="text-x-black font-x-thin text-base">
                        {{ __('Email') }} (*)
            ...
            <neo-stepper-item step="step-2" class="grid grid-cols-1 grid-rows-1 gap-6">
                ...
                <div class="flex flex-col gap-1">
                    ...
                    <neo-textarea placeholder="{{ __('Address') }}" name="address"
                        value="{{ old('address', $data->address) }}" rows="4"></neo-textarea>
                </div>
            </neo-stepper-item>
